...
ADR directory for this project: **`docs/adr`** (configured in `.forge/config.json`).
...
## 3. Format (`docs/adr/NNNN-short-topic.md`)
...
All ADRs sit under **`docs/adr/`** even when scoped to one area—the decision
usually crosses layers.

---

## 5. Status index (`docs/adr/README.md`)
...
This change is recorded as ADR-NNNN (docs/adr/NNNN-short-topic.md).
```
...
Short, focused ADRs embed well. **Do not** add `docs/adr/` to
...
- Use this file + `docs/adr/README.md` + `docs/adr/NNNN-*.md`.
