...
assert.equal(couldMatch("Quarterly Report.pdf", "qrp"), true, "a scattered subsequence still matches")
assert.equal(couldMatch("Quarterly Report.pdf", "prq"), false, "out of order is not a subsequence")
assert.equal(couldMatch("notes.md", ""), true, "an empty search rejects nothing")
assert.equal(couldMatch("notes.md", "notes.mdx"), false, "a search longer than the text cannot match")
